...
var __webpack_modules__ = {
    ...
    "../../../compiled/memfs/index.js": function(module) {
        "use strict";
        module.exports = import("../compiled/memfs/index.js").then(function(module) {
            return module;
        });
    },
...
for(var __webpack_i__ in (()=>{
    "use strict";
    let swcHelpersPath, pluginHelper_htmlPlugin, cssExtractPlugin, isDockerCached, cachedResult, hmrClientPath, overlayClientPath, mountPoint, defaultMountPoint;
    ...
    let prettyTime = (seconds)=>{
        ...
        if (seconds < 10) {
            ...
            return `${time = seconds.toFixed(digits), picocolors_index_js_default().bold(time)} s`;
        }
        if (seconds < 60) {
            let time;
            return `${time = seconds.toFixed(1), picocolors_index_js_default().bold(time)} s`;
        }
        let minutes = Math.floor(seconds / 60), minutesLabel = `${(time = minutes.toFixed(0), picocolors_index_js_default().bold(time))} m`, remainingSeconds = seconds % 60;
        if (0 === remainingSeconds) return minutesLabel;
        let secondsLabel = `${(time1 = remainingSeconds.toFixed(+(remainingSeconds % 1 != 0)), picocolors_index_js_default().bold(time1))} s`;
    ...
    isDebug() && (index_js_namespaceObject.logger.level = 'verbose'), index_js_namespaceObject.logger.override({
        debug: (message, ...args)=>{
            ...
            let time = picocolors_index_js_default().gray((hours = String((now = new Date()).getHours()).padStart(2, '0'), minutes = String(now.getMinutes()).padStart(2, '0'), seconds = String(now.getSeconds()).padStart(2, '0'), `${hours}:${minutes}:${seconds}`));
...
${section.body}` : section.body).join("\n\n"));
    ...
    async function initRsbuildConfig({ context, pluginManager }) {
        ...
        let normalizedBaseConfig = ((config)=>{
            ...
            let defaultEntry, { environments, dev, server: _server, provider: _provider, ...baseConfig } = normalizedConfig, isEnvironmentEnabled = (name)=>!specifiedEnvironments || specifiedEnvironments.includes(name), applyEnvironmentDefaultConfig = (config)=>{
                var root;
                let entryFile;
                config.source.entry && 0 !== Object.keys(config.source.entry).length || (config.source.entry = (defaultEntry || (root = rootPath, defaultEntry = (entryFile = findExists([
                    'ts',
                    'js',
                    'tsx',
                    'jsx',
        ...
        return tsconfigPaths.size && 'prefer-tsconfig' === normalizedBaseConfig.resolve.aliasStrategy && normalizedBaseConfig.dev.watchFiles.push({
            ...
            environments
        }, await updateEnvironmentContext(context, environments), distPaths = Object.values((context1 = context).environments).map((item)=>item.distPath), context1.distPath = getCommonParentPath(distPaths), ((config)=>{
    ...
    function formatFileList(paths, rootPath) {
        ...
        let fileInfo = files.slice(0, 1).map((file)=>{
            ...
            return originalFilePath = file, prefix = (root = rootPath).endsWith(external_node_path_.sep) ? root : root + external_node_path_.sep, (filePath = originalFilePath).startsWith(prefix) && (filePath = filePath.slice(prefix.length)), (parts = filePath.split(external_node_path_.sep).filter(Boolean)).length > 3 ? parts.slice(-3).join(external_node_path_.sep) : parts.join(external_node_path_.sep);
        }).join(', ');
    ...
    async function printFileSizes(options, stats, rootPath, distPath, environmentName) {
        ...
        if (showDetail) {
            var maxFileLength, maxSizeLength, fileHeader1, showGzipHeader;
            let lengths, rowTypes, headerRow, maxFileLength1 = Math.max(...assets.map((a)=>(a.folder + external_node_path_default().sep + a.name).length), showTotal ? totalSizeLabel.length : 0, fileHeader.length), maxSizeLength1 = Math.max(...assets.map((a)=>a.sizeLabel.length), totalSizeStr.length), showGzipHeader1 = !!(options.compressed && assets.some((item)=>null !== item.gzippedSize));
            for (let asset of (logs.push((maxFileLength = maxFileLength1, maxSizeLength = maxSizeLength1, fileHeader1 = fileHeader, showGzipHeader = showGzipHeader1, lengths = [
                maxFileLength,
                maxSizeLength
            ], rowTypes = [
                fileHeader1,
            ...
            }, ''), picocolors_index_js_default().blue(headerRow))), assets)){
                ...
                sizeLength < maxSizeLength1 && (sizeLabel += ' '.repeat(maxSizeLength1 - sizeLength));
                ...
                fileNameLength < maxFileLength1 && (fileNameLabel += ' '.repeat(maxFileLength1 - fileNameLength));
            ...
            if (showTotal) {
                ...
                if (customTotal) logs.push(customTotal);
                    ...
                    let log = '';
                    if (log += ' '.repeat(maxFileLength1 - totalSizeLabel.length), log += picocolors_index_js_default().magenta(totalSizeLabel), log += `   ${totalSizeStr}`, options.compressed) {
                        let colorFn = getAssetColor(totalGzipSize / assets.length);
                        log += ' '.repeat(maxSizeLength1 - totalSizeStr.length), log += `   ${colorFn(calcFileSize(totalGzipSize))}`;
                    }
                    logs.push(log);
                }
            }
    ...
    }, is_wsl = external_node_process_namespaceObject.env.__IS_WSL_TEST__ ? isWsl : isWsl(), wslDrivesMountPoint = (defaultMountPoint = '/mnt/', async function() {
    ...
    let CACHE_CONTROL_NO_CACHE_REGEXP = /(?:^|,)\s*?no-cache\s*?(?:,|$)/, statuses = {
        400: 'Bad Request',
        403: 'Forbidden',
        404: 'Not Found',
            ...
            function sendError(status, options) {
                let content = statuses[status] || String(status), document1 = Buffer.from(`<!DOCTYPE html>\n<html lang="en">\n<head>\n<meta charset="utf-8">\n<title>Error</title>\n</head>\n<body>\n<pre>${function(input) {
                    ...
                    for({ index } = match; index < input.length; index++){
                        ...
                        lastIndex !== index && (html += input.substring(lastIndex, index)), lastIndex = index + 1, html += htmlEntity;
                    }
                    return lastIndex !== index ? html + input.substring(lastIndex, index) : html;
                }(content)}</pre>\n</body>\n</html>`, 'utf-8'), headers = res.getHeaderNames();
                for(let i = 0; i < headers.length; i++)res.removeHeader(headers[i]);
                if (options?.headers) {
                    let keys = Object.keys(options.headers);
                    for(let i = 0; i < keys.length; i++){
                        let key = keys[i], value = options.headers[key];
                        void 0 !== value && res.setHeader(key, value);
                    }
                }
                res.statusCode = status, res.setHeader('Content-Type', 'text/html; charset=utf-8'), res.setHeader('Content-Security-Policy', "default-src 'none'"), res.setHeader('X-Content-Type-Options', 'nosniff');
            ...
            ready(async function() {
                ...
                if (req.headers['if-match'] || req.headers['if-unmodified-since'] || req.headers['if-none-match'] || req.headers['if-modified-since']) {
                    if (function() {
                        ...
                        return !1;
                    }()) return void sendError(412);
                    if (404 === res.statusCode && (res.statusCode = 200), (res.statusCode >= 200 && res.statusCode < 300 || 304 === res.statusCode) && function(resHeaders) {
                ...
                let [start1, end2] = (end1 = Math.max(offset = offset1, offset + len - 1), [
                    offset,
                    end1
                ]);
                try {
                ...
                let cleanup = ()=>{
                    var stream, suppress;
                    suppress = !0, 'function' == typeof (stream = bufferOrStream).destroy && stream.destroy(), 'function' == typeof stream.close && stream.on('open', function(fd) {
    ...
    class SocketServer {
        ...
        sendStats({ force = !1, token }) {
            ...
            let { statsJson, root: root1 } = result, newInitialChunks = new Set();
            ...
            if (statsJson.hash && this.sockWrite({
                ...
                this.sockWrite({
                    ...
                    data: {
                        ...
                        html: (errors = formattedErrors, root = root1, htmlItems = errors.map((item)=>{
                            var text, text1, text2, root1;
                            let ansiCodes, ret, pathRegex, urlRegex;
                            return text1 = (text = item) ? text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;') : '', ansiCodes = [], ret = text1.replace(/\x1B\[([0-9;]+)m/g, (_match, sequences)=>{
                                let style = '';
                            ...
                            }), ansiCodes.length > 0 && (ret += Array(ansiCodes.length + 1).join('</span>')), text2 = ret, root1 = root, pathRegex = /(?:\.\.?[/\\]|[a-zA-Z]:\\|\/)[^:]*:\d+:\d+/g, urlRegex = /(https?:\/\/(?:[\w-]+\.)+[a-z0-9](?:[\w-.~:/?#[\]@!$&'*+,;=])*)/gi, text2.split('\n').map((line)=>{
                                let replacedLine = line.replace(pathRegex, (file)=>{
                                    var base, filepath;
                                    let relativePath, hasClosingSpan = file.includes('</span>') && !file.includes('<span'), filePath = hasClosingSpan ? file.replace('</span>', '') : file, isAbsolute = external_node_path_default().isAbsolute(filePath), absolutePath = root1 && !isAbsolute ? external_node_path_default().join(root1, filePath) : filePath, relativePath1 = root1 && isAbsolute ? (base = root1, filepath = filePath, '' === (relativePath = (0, external_node_path_.relative)(base, filepath)) ? `.${external_node_path_.sep}` : relativePath.startsWith('.') ? relativePath : `.${external_node_path_.sep}${relativePath}`) : filePath;
...
`)
    ...
    let faviconFallbackMiddleware = (req, res, next)=>{
        '/favicon.ico' === req.url ? (res.statusCode = 204, res.end()) : next();
            ...
            on_finished_default()(res, ()=>{
                var status;
                let method = req.method, url = req.originalUrl || req.url, status1 = Number(res.statusCode), statusColor = (status = status1) >= 500 ? picocolors_index_js_default().red : status >= 400 ? picocolors_index_js_default().yellow : status >= 300 ? picocolors_index_js_default().cyan : status >= 200 ? picocolors_index_js_default().green : (res)=>res, endAt = process.hrtime(), totalTime = (endAt[0] - _startAt[0]) * 1e3 + (endAt[1] - _startAt[1]) * 1e-6;
                index_js_namespaceObject.logger.debug(`${statusColor(status1)} ${method} ${url} ${picocolors_index_js_default().dim(`${totalTime.toFixed(3)} ms`)}`);
            }), next();
    ...
    }, postfixRE = /[?#].*$/, getBaseMiddleware = ({ base })=>(req, res, next)=>{
  ...
  - Actual: ${options?.action}`);
    ...
    class RsbuildProdServer {
        ...
        async applyDefaultMiddlewares() {
            ...
            if ('verbose' === index_js_namespaceObject.logger.level && this.middlewares.use(getRequestLoggerMiddleware()), cors) {
                let { default: corsMiddleware } = await Promise.resolve().then(__webpack_require__.bind(__webpack_require__, "../../compiled/cors/index.js"));
...
`)
            ...
            let exposedKeys, { hooks } = context, publicContext = (exposedKeys = [
                'action',
                'version',
                'rootPath',
                'distPath',
                ...
                setup (api) {
                    api.modifyBundlerChain((chain, { isProd, environment })=>{
                        let { config } = environment, getMergedFilename = (assetType)=>{
                            ...
                            var exts1;
                            let matcher, regExp = (matcher = (exts1 = exts).map((ext)=>ext.trim()).map((ext)=>ext.startsWith('.') ? ext.slice(1) : ext).join('|'), RegExp(1 === exts1.length ? `\\.${matcher}$` : `\\.(?:${matcher})$`, 'i')), { dataUriLimit } = config.output, maxSize = 'number' == typeof dataUriLimit ? dataUriLimit : dataUriLimit[assetType];
                ...
                setup (api) {
                    api.modifyBundlerChain(async (chain, { HtmlPlugin, CHAIN_ID, environment })=>{
                        ...
                        let assetPrefix = getPublicPathFromChain(chain, !1), entries = chain.entryPoints.entries() || {}, entryNames = Object.keys(entries).filter((entryName)=>!!htmlPaths[entryName]), extraDataMap = new Map(), finalOptions = await Promise.all(entryNames.map(async (entryName)=>{
                            var entryName1, entryName2, config1, assetPrefix1, entryName3, config2, templateContent, entryName4, entryName5;
                            let metaTags, entryValue = entries[entryName].values(), chunks = function(entryName, entryValue) {
                                let chunks = [
                                    entryName
                                ];
                            ...
                            })), filename = htmlPaths[entryName], { templatePath, templateContent: templateContent1 } = await getTemplate(entryName, config, api.context.rootPath), templateParameters = (entryName2 = entryName, config1 = config, assetPrefix1 = assetPrefix, (compilation, assets, assetTags, pluginOptions)=>{
                                ...
                                meta: (entryName3 = entryName, config2 = config, templateContent = templateContent1, metaTags = reduceConfigsMergeContext({
                                    initial: {},
                                    config: config2.html.meta,
                                    ctx: {
                                        entryName: entryName3
                                    }
                                }), templateContent && metaTags.charset && /<meta[^>]+charset=["'][^>]*>/i.test(templateContent) && delete metaTags.charset, metaTags),
                                chunks,
                                inject,
                                filename,
                                entryName,
                            ...
                            extraDataMap.set(entryName, extraData), templateContent1 && (extraData.templateContent = templateContent1);
                            ...
                            tagConfig && (extraData.tagConfig = tagConfig), entryName4 = entryName, pluginOptions.title = reduceConfigsMergeContext({
                                initial: '',
                            ...
                            let favicon = (entryName5 = entryName, reduceConfigsMergeContext({
                                initial: '',
                                config: config.html.favicon,
                                ctx: {
                                    entryName: entryName5
                                }
                            }));
                            ...
                            return finalOptions.template || finalOptions.templateContent || (pluginOptions.template = '', pluginOptions.templateContent = templateContent1), finalOptions;
                        }));
...
`;
                ...
                setup (api) {
                    api.modifyBundlerChain({
                        ...
                        handler: (chain, { CHAIN_ID, isDev, isProd, target, environment })=>{
                            var swcConfig, swcConfig1, polyfillMode;
                            let finalPluginImport, targets, { config, browserslist } = environment, cacheRoot = external_node_path_default().join(api.context.cachePath, '.swc'), rule = chain.module.rule(CHAIN_ID.RULE.JS).test(SCRIPT_REGEX).type("javascript/auto").dependency({
                                not: 'url'
                            }).resourceQuery({
                                not: RAW_QUERY_REGEX
                            });
                            ...
                            let swcConfig2 = function({ browserslist, cacheRoot, config, isProd }) {
                                return {
                                    jsc: {
                                        externalHelpers: !0,
                                        parser: {
                            ...
                            if (swcConfig = swcConfig2, finalPluginImport = ((options)=>{
                                if (!options) return [];
                                let imports = [];
                                for (let item of helpers_castArray(options))isFunction(item) ? imports = item(imports) ?? imports : imports.push(item);
                                return imports;
                            })(config.source.transformImport), finalPluginImport?.length && (swcConfig.rspackExperiments ??= {}, swcConfig.rspackExperiments.import ??= [], swcConfig.rspackExperiments.import.push(...finalPluginImport)), function(swcConfig, config) {
                            ...
                            }(swcConfig2, config), (targets = helpers_castArray(target)).includes('web') || targets.includes('web-worker')) {
                                let polyfillMode1 = config.output.polyfill;
                                if ('off' === polyfillMode1) swcConfig2.env.mode = void 0;
                                else {
                                    let coreJsPath, version, coreJsDir;
                                    swcConfig2.env.mode = polyfillMode1;
                                    let coreJsDir1 = (swcConfig1 = swcConfig2, polyfillMode = polyfillMode1, version = ((corejsPkgPath)=>{
                                        try {
                                    ...
                                    })(coreJsPath = swc_require.resolve('core-js/package.json')), coreJsDir = external_node_path_default().dirname(coreJsPath), swcConfig1.env.coreJs = version, 'usage' === polyfillMode && (swcConfig1.env.shippedProposals = !0), coreJsDir);
                                    for (let item of [
                                        rule,
                                        dataUriRule
                                    ])item.resolve.alias.set('core-js', coreJsDir1);
                                }
                            }
                    ...
                    api.onAfterStartDevServer(onStartServer), api.onAfterStartProdServer(onStartServer), api.onBeforeBuild(async ({ isFirstCompile, environments })=>{
                        if (isFirstCompile) for (let publicDir of normalizePublicDirs(api.getNormalizedConfig().server.publicDir)){
                            ...
                            try {
                                ...
                                throw err instanceof Error && (err.message = `Failed to copy public directory '${picocolors_index_js_default().yellow(normalizedPath)}' to output directory. To disable public directory copying, set \`${picocolors_index_js_default().cyan('server.publicDir: false')}\` in your config.\n${err.message}`), err;
                            }
                        }
                    });
                }
        ...
        let provider = config.provider || rspackProvider, providerInstance = await provider({
            ...
            if (checkDistDir) {
                ...
                if (path = distPath, 0 === (files = external_node_fs_default().readdirSync(path)).length || 1 === files.length && '.git' === files[0]) throw Error(`${picocolors_index_js_default().dim('[rsbuild:preview]')} The output directory ${picocolors_index_js_default().yellow(distPath)} is empty, please build the project before previewing.`);
            }
    ...
    async function loadConfig_loadConfig({ cwd = process.cwd(), path, envMode, meta, loader = 'jiti' } = {}) {
        ...
        if ('native' === loader || 'auto' === loader && (process.features.typescript || process.versions.bun || process.versions.deno) || /\.(?:js|mjs|cjs)$/.test(configFilePath)) try {
            ...
            let errorMessage = `Failed to load file with native loader: ${picocolors_index_js_default().dim(configFilePath)}`;
            if ('native' === loader) throw index_js_namespaceObject.logger.error(errorMessage), err;
            index_js_namespaceObject.logger.debug(`${errorMessage}, fallback to jiti.`), index_js_namespaceObject.logger.debug(err);
        }
        try {
    ...
    function runCLI() {
        ...
        if (-1 !== logLevelIndex) {
            ...
            level && [
                ...
                'error',
                'silent'
            ].includes(level) && !isDebug() && (index_js_namespaceObject.logger.level = level);
        }
        !function() {
        ...
        try {
            ...
            (cli = ((name = "")=>new CAC(name))('rsbuild')).version("1.5.13"), cli.option('--base <base>', 'Set the base path of the server').option('-c, --config <config>', 'Set the configuration file (relative or absolute path)').option('--config-loader <loader>', 'Set the config file loader (auto | jiti | native)', {
                ...
                for (let section of (sections.shift(), sections))'Commands' === section.title && (section.body = section.body.replace(`         ${devDescription}`, `dev      ${devDescription}`)), section.title?.startsWith('For more info') ? (section.title = picocolors_index_js_default().dim('  For details on a sub-command, run'), section.body = picocolors_index_js_default().dim('  $ rsbuild <command> -h')) : section.title = picocolors_index_js_default().cyan(section.title);
            }), cli.parse();
        } catch (err) {
