...
import { unescape as external_node_querystring_unescape } from "node:querystring";
var superClass, superClass1, __webpack_modules__ = {
    ...
    let normalizedBaseConfig = ((config)=>{
        ...
        let defaultEntry, { environments, dev, server: _server, provider: _provider, ...baseConfig } = normalizedConfig, isEnvironmentEnabled = (name)=>!specifiedEnvironments || specifiedEnvironments.includes(name), applyEnvironmentDefaultConfig = (config)=>{
            ...
            config.source.entry && 0 !== Object.keys(config.source.entry).length || (config.source.entry = (defaultEntry || (defaultEntry = (entryFile = findExists([
                ...
                'mjs',
                'cjs'
            ].map((ext)=>(0, external_node_path_.join)(rootPath, `src/index.${ext}`)))) ? {
                index: entryFile
            } : {}), defaultEntry));
...
function formatFileList(paths, rootPath) {
    ...
    let fileInfo = files.slice(0, 1).map((file)=>{
        ...
        return prefix = rootPath.endsWith(external_node_path_.sep) ? rootPath : rootPath + external_node_path_.sep, (filePath = file).startsWith(prefix) && (filePath = filePath.slice(prefix.length)), (parts = filePath.split(external_node_path_.sep).filter(Boolean)).length > 3 ? parts.slice(-3).join(external_node_path_.sep) : parts.join(external_node_path_.sep);
    }).join(', ');
...
async function printFileSizes(options, stats, rootPath, distPath, environmentName) {
    ...
    if (showDetail) {
        let lengths, rowTypes, headerRow, maxFileLength = Math.max(...assets.map((a)=>(a.folder + external_node_path_.default.sep + a.name).length), showTotal ? totalSizeLabel.length : 0, fileHeader.length), maxSizeLength = Math.max(...assets.map((a)=>a.sizeLabel.length), totalSizeStr.length), showGzipHeader = !!(options.compressed && assets.some((item)=>null !== item.gzippedSize));
        for (let asset of (logs.push((lengths = [
            maxFileLength,
            maxSizeLength
        ], rowTypes = [
            fileHeader,
...
let noop = ()=>{}, assets_middleware_require = (0, external_node_module_.createRequire)(import.meta.url), assetsMiddleware = async ({ config, compiler, context, socketServer, resolvedPort })=>{
    ...
    let outputFileSystem = await setupOutputFileSystem(writeToDisk, compilers), instance = (ready = (callback)=>{
        ...
        ready(async function() {
            ...
            let [start, end1] = (end = Math.max(offset = offset1, offset + len - 1), [
                offset,
                end
            ]);
            try {
            ...
            let cleanup = ()=>{
                var stream;
                'function' == typeof (stream = bufferOrStream).destroy && stream.destroy(), 'function' == typeof stream.close && stream.on('open', function(fd) {
                    'number' == typeof fd && this.close();
                }), 'function' == typeof stream.addListener && (stream.removeAllListeners('error'), stream.addListener('error', ()=>{}));
            };
...
`)
    ...
    logger.debug('registering default plugins'), pluginManager.addPlugins([
        {
            name: 'rsbuild:basic',
            setup (api) {
            ...
            setup (api) {
                api.modifyBundlerChain((chain, { isProd, environment })=>{
                    let { config } = environment, getMergedFilename = (assetType)=>{
                        ...
                        let matcher, regExp = (matcher = exts.map((ext)=>ext.trim()).map((ext)=>ext.startsWith('.') ? ext.slice(1) : ext).join('|'), RegExp(1 === exts.length ? `\\.${matcher}$` : `\\.(?:${matcher})$`, 'i')), { dataUriLimit } = config.output, maxSize = 'number' == typeof dataUriLimit ? dataUriLimit : dataUriLimit[assetType];
                ...
                api.modifyBundlerChain(async (chain, { HtmlPlugin, CHAIN_ID, environment })=>{
                    ...
                    let assetPrefix = getPublicPathFromChain(chain, !1), entries = chain.entryPoints.entries() || {}, entryNames = Object.keys(entries).filter((entryName)=>!!htmlPaths[entryName]), extraDataMap = new Map(), finalOptions = await Promise.all(entryNames.map(async (entryName)=>{
                        let metaTags, entryValue = entries[entryName].values(), chunks = function(entryName, entryValue) {
                            ...
                            meta: (metaTags = reduceConfigsMergeContext({
                                initial: {},
            ...
            setup (api) {
                api.modifyBundlerChain({
                    ...
                    handler: (chain, { CHAIN_ID, isDev, isProd, target, environment })=>{
                        let finalPluginImport, targets, { config, browserslist } = environment, cacheRoot = external_node_path_.default.join(api.context.cachePath, '.swc'), rule = chain.module.rule(CHAIN_ID.RULE.JS).test(SCRIPT_REGEX).type("javascript/auto").dependency({
                            not: 'url'
                        }).resourceQuery({
                            not: RAW_QUERY_REGEX
                        });
                        ...
                        let swcConfig = function({ browserslist, cacheRoot, config, isProd }) {
                            return {
                                jsc: {
                                    externalHelpers: !0,
                                    parser: {
                        ...
                        if (finalPluginImport = ((options)=>{
                            ...
                            if ('off' === polyfillMode) swcConfig.env.mode = void 0;
                                ...
                                let coreJsDir1 = (version = ((corejsPkgPath)=>{
                                    try {
                                ...
                                })(coreJsPath = swc_require.resolve('core-js/package.json')), coreJsDir = external_node_path_.default.dirname(coreJsPath), swcConfig.env.coreJs = version, 'usage' === polyfillMode && (swcConfig.env.shippedProposals = !0), coreJsDir);
                                for (let item of [
                                    rule,
                                    dataUriRule
            ...
            setup (api) {
                ...
                api.onAfterStartDevServer(onStartServer), api.onAfterStartProdServer(onStartServer), api.onBeforeBuild(async ({ isFirstCompile, environments })=>{
                    if (isFirstCompile) for (let publicDir of normalizePublicDirs(api.getNormalizedConfig().server.publicDir)){
                        ...
                        try {
                            await Promise.all(distPaths.map(async (distPath)=>(isDeno && node_fs.existsSync(distPath) && await node_fs.promises.rm(distPath, {
                                    recursive: !0,
                                    force: !0
                                }), node_fs.promises.cp(normalizedPath, distPath, {
                                    recursive: !0,
                                    dereference: !0
                                }))));
                        } catch (err) {
